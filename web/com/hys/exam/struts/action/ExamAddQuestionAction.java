 ...
 * 说明:
 */
public class ExamAddQuestionAction extends BaseAction {
	...
	//		for (int i = 0; i < qform.getAnswer_content().length; i++) {
				ExamQuestionKey key = new ExamQuestionKey();
				key.setContent(qform.getContent());
				key.setSeq(1);
				keyList.add(key);
	//		}
		...
		ExamQuestion eq = localExamQuestionFacade.addQuestion(quest);
		...
		idList.add(eq.getId());
		localExamQuestionFacade.addQuestionProp(quest.getQuestionPropMap(), idList, null, quest.getState());
