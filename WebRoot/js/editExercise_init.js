...
function setUser() {
	if(userNamesId.length > 0 && userNamesId != null)
	{
	    ...
	    //数据率重
	    userNamesInfo=sort(userNamesInfo);
...
function deleteUser(userId){
	...
	for(var i=0; i<userNamesId1.length;i++){
		if(userId == userNamesId1[i]){
			userNamesId1.splice(i,1);
			$("#userNames_tr_"+userId).remove();
			break;
		}
	}
	...
	for(var j=0; j<userNamesInfo.length;j++) {
	    if(userId == userNamesInfo[j][6]){
	    	userNamesInfo.splice(j,1);
	      break;
	    }
	}
...
//	    for (var i in pStr) {
	    for(var i=0;i<orgNamesId1.length;i++){
    ...
    if (confirm("确认删除吗？")) {
        ...
        $.ajax({
            ...
            success: function (data) {
                $.each(data.list, function (i, item) {
                	for(var i= 0; i< myUserNamesId.length;i++){
                		if(item.userId == myUserNamesId[i].split("+")[0]){
                			myUserNamesId.splice(i, 1);
                			break;
                		}
                	}
                	for(var i= 0; i< userNamesInfo.length;i++){
                		if(item.userId == userNamesInfo[i][6]){
                			userNamesInfo.splice(i, 1);
                			break;
                		}
                	}
