...
if(idCard != '' && (idCard.length == 15 || idCard.length == 18)){
	if(idCard.length == 15){
		...
		birthday = '19'+arr_data[2]+'-'+arr_data[3]+'-'+arr_data[4];

	}else if(idCard.length == 18){
		var re_eighteen = /^(\d{6})(\d{4})(\d{2})(\d{2})(\d{3})([0-9]|X)$/;
		var arr_data = idCard.match(re_eighteen);
		birthday = arr_data[2]+'-'+arr_data[3]+'-'+arr_data[4];
	}
}
return birthday;
}
...
(function($,win,undef){
	...
	Validform.util={
		...
		//是否在长度范围,字节数
		isLengthRange:function(){
			var obj=this;
			var res = true;
			
			//字数范围验证,有atr时校验,无时不校验   2013/02/01 by joe.kwok
			var len = getByteLength(obj.val());
		...
		_regcheck:function(datatype,gets,obj,curform){
			...
			/*
			// 验证字符串字节数长度是否在范围内joe.kwok
			if(!Validform.util.isLengthRange.call(obj)){
				passed=false;
			}
			*/
