...
(function(win) {
	...
	*/
    ...
    //初始化日期
    jeDt.nowDate = function(timestamp, format, custom) {
        var De = new Date(timestamp | 0 ? function(tamp) {
            return tamp < 864e5 ? +new Date() + tamp * 864e5 :tamp;
        }(parseInt(timestamp)) :+new Date());
        /****** custom ******/
        if(custom){
        	if(custom=="start_date"){
        		return jeDt.parse([ De.getFullYear(), De.getMonth() + 1, De.getDate() ], [ 00, 00, 00 ], format);
        	}else if(custom=="end_date"){
        		return jeDt.parse([ De.getFullYear(), De.getMonth() + 1, De.getDate() ], [ 23, 59, 59 ], format);
        	}
        }
        /****** custom ******/
    ...
    InitDate.prototype = {
        init:function() {
            ...
            var customStr = null;
            if(opts.customDate){
            	customStr = opts.customDate;
            }
            /****** custom ******/
            var nowDateVal = jeDt.nowDate(null, opts.format, /*custom*/customStr/*custom*/);
            if (opts.isinitVal) {
