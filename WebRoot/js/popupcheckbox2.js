...
var PopupSelector = {
    ...
    popup: function(type, ref, selectdtids, tagPrefix, checkType) {
    	...
    	//alert("调用开始了嘿嘿!!!!!!!!!!!!!!!!") ;
    	//alert("_selItems-->" + this._selItems[this._type]) ;
    	if(this._selItems[this._type].length==0) {
    		var str;
	    	var dat;
			if (type == "loc" ) {
		        str = jQuery("#renderSelIds").val();
		        //alert("renderSelIds-->" + str) ;
		        dat = getLoc();
		        //alert("dat-->" + dat) ;
		    }
		    if (type == "ind" ) {
		        str = jQuery("#questType_2").val();
		        dat = getInd();
		    }
		    if (type == "ind2" ) {
		        str = jQuery("#question_label_ids").val();
		        dat = getInd2();
		    }
		    if (type == "ind5" ) {
		        str = jQuery("#question_label_ids").val();
		    ...
		    if (type == "ind4" ) {
		        str = jQuery("#questCognize_8_2").val();
		        dat = getInd4("8");
		    }
		    if (type == "ind6" ) {
		        str = jQuery("#questTitle_9").val();
		        dat = getInd4("9");
		    }
		    ...
		    if(str != "") {
				...
				alert("弹出层不为空时的str-->" + str) ;
		    }
        }
        this.AarearenderSelItemEleId = selectdtids; //alert("selectdtids-->" +selectdtids) ;
        this.AareatagPrefix = tagPrefix; //alert("tagPrefix-->" +tagPrefix) ;
        this.AarearenderSelItemDiv = ref; //alert("ref-->" +ref + "----ref.id--" + ref.id) ;
        var selLocIDs;

        //alert(" this._box --> " + this._box + "-------popupSelector-->" + jQuery("#popupSelector")) ;
        if (this._box == null) this._box = jQuery("#popupSelector"); 
        
        //alert(" this._subBox --> " + this._subBox + "-------subItems-->" + document.getElementById("subItems")) ;
    ...
    render: function(event) {
        ...
        if (this._type == "loc") {
           /* jQuery("#subHeader1").html("<span>一级学科：</span>");
        	...
        	if(this._type == "ind"){
            	start = 0;
	            jQuery("#subHeader1").html("<span>新世界题库：</span>");
	            jQuery("#subHeader2").html("<span>本院题库：</span>").show();
        ...
        //var ql_ids = jQuery("#question_label_ids").val();
        ...
        for (var i = start; i <= end; i++) {
            ...
            if (parItem.subItems && parItem.subItems.length >= 1) {
                html.push("<li id=jQuery" + i + " name=" + parItem.id + " onclick=\"PopupSelector.showSubItems(");
                html.push(i);
            	...
            	if(this._type == "ind2"){
            		html.push("<tr>");
            		 if (this._curItems.has({ id: parItem.id, name: parItem.name, parObj: null }))
	                    html.push("<td id=jQuery" + i + " name=" + parItem.id + " class=\"layon\" style='width:150;'>");
	                else
	                    html.push("<td id=jQuery" + i + " name=" + parItem.id + " class=\"nonelay\" style='width:150px;'>");
	               
	                html.push(dbcToSbc(parItem.name));
	               
	                html.push("</td>");
	                //html.push("<td id=jq" + parItem.id + " class=\"nonelay\" style='width:150px;'>");
	                //html.push("<span style='color:red;'>");
	                //html.push("总题量：");
	                //html.push(parItem.quesCount);
	                //html.push("</span>");
	                //html.push("</td>");
	                html.push("<td id=jq" + parItem.id + " class=\"nonelay\" style='width:130px;'>");
	                html.push("<input id='shu_count_"+parItem.id+"' type='hidden' value='"+parItem.quesCount+"' />");
	                html.push("<input type='hidden' value='" + parItem.id + "' />");
            		if (this._curItems.has({ id: parItem.id, name: parItem.name, parObj: null })){
            			for(var m=0; m<this._ql_ids.length; m++){
	            			if(this._ql_ids[m].id == parItem.id){
				                html.push("<input id='shu"+parItem.id+"' type='text' value='"+this._ql_ids[m].shu+"' class='shu' onfocus='checkTotalQues();' onblur='checkTotalQues();'/>");
	            			}
            			}
            		}else {
		                html.push("<input id='shu"+parItem.id+"' type='text' value='0' class='shu' onfocus='checkTotalQues();' onblur='checkTotalQues();'/>");
		                //html.push("<input id='fen"+parItem.id+"' type='text' value='1'/>");
            		}
            		...
            		if (this._curItems.has({ id: parItem.id, name: parItem.name, parObj: null })){
            			for(var m=0; m<this._ql_ids.length; m++){
	            			if(this._ql_ids[m].id == parItem.id){
				                //html.push("<input id='shu"+parItem.id+"' type='text' value='"+this._ql_ids[m].shu+"' class='shu'/>");
				                html.push("<input id='fen"+parItem.id+"' type='text' value='"+this._ql_ids[m].fen+"' class='fen' onfocus='checkTotalQues();' onblur='checkTotalQues();'/>");
	            			}
            			}
            		}else {
		                //html.push("<input id='shu"+parItem.id+"' type='text' value='0'/>");
		                html.push("<input id='fen"+parItem.id+"' type='text' value='1' onfocus='checkTotalQues();' onblur='checkTotalQues();'/>");
            		}
	                html.push("</span>");
	                html.push("</td>");
					...
					html.push("<input id='shu_count_" + d.id
							+ "' type='hidden' value='" + d.quesCount + "' />");
        ...
        if (this._type == "ind" || this._type == "ind2" || this._type == "ind3"|| this._type == "ind4"||this._type == "ind6"|| this._type == "ind5" || this._type == "comptype") {
    ...
    OK: function() {
	        ...
	        jQuery.each(this._curItems, function() {
	            ...
	            if (this.parObj && parthis._type == "loc") {
	                ...
	                if (this.parObj != null && null == this.parObj.pparObj) {
	                    ...
	                    idstemp.push(parthis.allItems[parthis._type]()[this.parObj.ppparObj].id + "-" + parthis.allItems[parthis._type]()[this.parObj.ppparObj].subItems[this.parObj.pparObj].id + "-" + this.parObj.id + "-" + this.id);
	                	...
	                	idstemp.push(parthis.allItems[parthis._type]()[this.parObj.pppparObj].id + "-" + parthis.allItems[parthis._type]()[this.parObj.pppparObj].subItems[this.parObj.ppparObj].id + "-" + parthis.allItems[parthis._type]()[this.parObj.pppparObj].subItems[this.parObj.ppparObj].subItems[this.parObj.pparObj].id + "-" + this.parObj.id + "-" + this.id);
	                }
		        	...
		        	html2.push(this.id);
		        	html2.push("'>");
		        	html2.push("<input class='input_loc' onfocus='checkTotalbfb();' onblur='checkTotalbfb();'  type='text' id='");
		        	...
		        	html2.push(this.id);
		        	html2.push("'>");
		        	html2.push("<input onfocus='checkTotalbfb();' onblur='checkTotalbfb();' class='input_loc' size='10' type='text' id='");
		        	html2.push(this.id);
		        	html2.push("'>");
		        	html2.push("%");
		        	html2.push("</td>");
        ...
        if(parthis._type == "ind2"){
        	if(sele_ques_num>0){
		        ...
		        if(eval(p_num_sum)<eval(p_num)){
		        	alert("合计试题数量： "+p_num_sum+" 道，试卷总题量： "+p_num+" 道，还差 "+eval(eval(p_num)-eval(p_num_sum))+" 道，请调整题量！");
		        	isQueNum = true;
		            return false;
		        }
		        var p_score = jQuery("#paperScore").val();
		        if(eval(p_score_sum)>eval(p_score)){
		        	alert("合计试卷分数： "+p_score_sum+" 分，超过了试卷总分数： "+p_score+" 分，请调整分数！");
		        	isQueNum = true;
		        }
		        if(eval(p_score_sum)<eval(p_score)){
		        	alert("合计试卷分数： "+p_score_sum+" 分，试卷总分数： "+p_score+" 分，还差 "+eval(eval(p_score)-eval(p_score_sum))+" 分，请调整分数！");
		        	isQueNum = true;
		        }
        ...
        if (parthis._type == "ind5") {
			if (sele_ques_num > 0) {
				...
				var p_num = jQuery("#questionNum").val();
				if (eval(p_num_sum) > eval(p_num)) {
					alert("合计试题数量： " + p_num_sum + " 道，超过了试卷总题量： " + p_num
							+ " 道，请调整题量！");
					isQueNum = true;
				}
				if (eval(p_num_sum) < eval(p_num)) {
					alert("合计试题数量： " + p_num_sum + " 道，试卷总题量： " + p_num
							+ " 道，还差 " + eval(eval(p_num) - eval(p_num_sum))
							+ " 道，请调整题量！");
					isQueNum = true;
					return false;
				}
				...
				if (eval(p_score_sum) < eval(p_score)) {
					alert("合计试卷分数： " + p_score_sum + " 分，试卷总分数： " + p_score
							+ " 分，还差 "
							+ eval(eval(p_score) - eval(p_score_sum))
							+ " 分，请调整分数！");
					isQueNum = true;
				}
			} else {
        ...
        if (this._curItems) {
            if (this._type == "loc" ) {
                jQuery("#renderSelIds").val(ids.join(','));
            }
            if (this._type == "ind" ) {
                jQuery("#questType_2").val(ids.join(','));
            }
            if (this._type == "ind2" ) {
                jQuery("#question_label_ids").val(ids.join(','));
            }
            if (this._type == "ind5" ) {
                jQuery("#question_label_ids").val(ids.join(','));
            }
            ...
            if (this._type == "ind4" ) {
                jQuery("#questCognize_8_2").val(ids.join(','));
            }
            if (this._type == "ind6" ) {
                jQuery("#questTitle_9").val(ids.join(','));
            }
            if (this._type == "cat" ) {
        ...
        if (this._type == "loc" ) {
        	...
        	html2.push("</td>");
        	html2.push("<td>");
        	html2.push("<input type='text' id='totalbfb' value='0' size='10' readonly>%");
        	html2.push("</td>");
		    html2.push("</tr>");
    ...
    empty: function() {
        ...
        jQuery(".shu").each(function(i) { this.value = 0; });
        jQuery(".fen").each(function(i) { this.value = 1; });
    ...
    checkAll: function() {
        ...
        jQuery("#allItems1 input").each(function(i) { 
        	this.checked = true;
        	var item = { id: this.value.split("@")[0], name: this.value.split("@")[1], parObj: null };
        	if(!custem.has(item)){
	        	custem.push(item);
        	}
        });
        if(this._type == 'ind'){
         ...
         jQuery("#allItems1 li").each(function(i) { if (this.className == "nonelay") this.className = "layon";});
         jQuery("#allItems2 li").each(function(i) { if (this.className == "nonelay") this.className = "layon";});
    },
    InSalaryPage: false,
    ...
    showtips: function() {
        if (this._curItems.length == 0) {
            if (this._type != "ind" && this._type != "ind2" && this._type != "ind3" && this._type != "ind4"&& this._type != "ind5"&& this._type != "ind6")
        ...
        function loadSubItems(index, ref, self) {
            ...
            for (var i = startIndex; i < parItem.subItems.length; i++) {
                ...
                if (ref.getElementsByTagName("input")[0].checked == true || self._curItems.has(subItem) || self._curItems.GetFather(subItem)) {
                    if (parItem.subItems[i].subItems && parItem.subItems[i].subItems.length > 0) {
                        html.push("<li id=^" + i + " name=" + parItem.id + "  class=\"layicon\"  onclick=\"PopupSelector.showThirdItems(");
                        html.push(index);
                        html.push(",")
                        html.push(i);
                        ...
                        html.push("<li id=^" + i + " name=" + parItem.id + " class=\"layon\" onclick='PopupSelector.clearThirdHidden()' onclick='PopupSelector.hideThirdItems(this)'>");
                    ...
                    if (parItem.subItems[i].subItems && parItem.subItems[i].subItems.length > 0) {
                        html.push("<li id=^" + i + " name=" + parItem.id + " onclick=\"PopupSelector.showThirdItems(");
                        html.push(index);
                        html.push(",")
                        html.push(i);
                        html.push(", this, true)\" onmouseout=\"PopupSelector.hideThirdItems(this)\">");
                        ...
                        html.push("<li id=^" + i + " name=" + parItem.id + " class=\"nonelay\" onclick='PopupSelector.clearThirdHidden()' onclick='PopupSelector.hideThirdItems(this)'>");
                        html.push("<a href=\"javascript:void(0);\"><label for=\"scbx");
                        html.push(subItem.id);
                        html.push("\">");
                        html.push("<input id=\"scbx");
    ...
    showThirdItems: function(index, secondIndex, ref, isDelay, ev) {
        ...
        function loadThirdItems(index, secondIndex, ref, self) {
			...
			//alert(ref.getElementsByTagName("input")) ;
            var parentChecked = (ref.getElementsByTagName("input")[0].checked == true) ? " checked disabled " : "";
            var item = { id: parItem.id, name: parItem.name, parObj: null };
            var html = [];
            html.push("<ol >");
            ...
            for (var i = startIndex; i < parItem.subItems.length; i++) {
                ...
                if (ref.getElementsByTagName("input")[0].checked == true || self._curItems.has(subItem) || self._curItems.GetFather(subItem)) {
                    if (parItem.subItems[i].subItems && parItem.subItems[i].subItems.length > 0) {
                        html.push("<li id=^^" + i + " name=" + parItem.id + "  class=\"layicon\"  onclick=\"PopupSelector.showFourthItems(");
                        html.push(index);
                        html.push(",")
                        ...
                        html.push("<li id=^^" + i + " name=" + parItem.id + " class=\"layon\" onclick='PopupSelector.clearFourthHidden()' onclick='PopupSelector.hideFourthItems(this)'>");
                    ...
                    if (parItem.subItems[i].subItems && parItem.subItems[i].subItems.length > 0) {
                        html.push("<li id=^^" + i + " name=" + parItem.id + " onclick=\"PopupSelector.showFourthItems(");
                        html.push(index);
                        html.push(",")
                        html.push(secondIndex);
                        html.push(",")
                        ...
                        html.push("<li id=^^" + i + " name=" + parItem.id + " class=\"nonelay\" onclick='PopupSelector.clearFourthHidden()' onclick='PopupSelector.hideFourthItems(this)'>");
                        html.push("<a href=\"javascript:void(0);\"><label for=\"tcbx");
                        html.push(subItem.id);
                        html.push("\">");
                        html.push("<input id=\"tcbx");
    ...
    showFourthItems: function(index, secondIndex,thirdIndex, ref, isDelay, ev) {
        ...
        function loadFourthItems(index, secondIndex,thirdIndex, ref, self) {
            ...
            for (var i = startIndex; i < parItem.subItems.length; i++) {
                ...
                if (ref.getElementsByTagName("input")[0].checked == true || self._curItems.has(subItem) || self._curItems.GetFather(subItem)) {
                    if (parItem.subItems[i].subItems && parItem.subItems[i].subItems.length > 0) {
                        html.push("<li id=^^^" + i + " name=" + parItem.id + "  class=\"layicon\"  onclick=\"PopupSelector.showFifthItems(");
                        html.push(index);
                        html.push(",")
                        html.push(secondIndex);
                        html.push(",")
                        ...
                        html.push("<li id=^^^" + i + " name=" + parItem.id + " class=\"layon\" onclick='PopupSelector.clearFifthHidden()' onclick='PopupSelector.hideFifthItems(this)'>");
                    ...
                    if (parItem.subItems[i].subItems && parItem.subItems[i].subItems.length > 0) {
                        html.push("<li id=^^^" + i + " name=" + parItem.id + " onclick=\"PopupSelector.showFifthItems(");
                        html.push(index);
                        html.push(",")
                        html.push(secondIndex);
                        html.push(",")
                        ...
                        html.push("<li id=^^^" + i + " name=" + parItem.id + " class=\"nonelay\" onclick='PopupSelector.clearFifthHidden()' onclick='PopupSelector.hideFifthItems(this)'>");
                        html.push("<a href=\"javascript:void(0);\"><label for=\"fcbx");
                        html.push(subItem.id);
                        html.push("\">");
                        html.push("<input id=\"fcbx");
    ...
    hideSubItems: function(ref) {
        ...
        if (this._showTimer) clearTimeout(this._showTimer);
        if (this._hideTimer) clearTimeout(this._hideTimer);
        this._hideTimer = setTimeout(function() { jQuery("#subItems").hide();
        }, 100);
    },
    ...
    _ref: null,
    _type: null,
    _types: ["cat", "ind","ind2","ind3","ind4","ind5","ind6", "loc", "comptype"],
    _maxSize: 15,
    _checkType: "checkbox",
    ...
    _curItems: [],
    _ql_ids: [],
    _selItems: { cat: [], loc: [], ind: [],ind2: [],ind3: [],ind4: [],ind5: [],ind6: [], comptype: [] },

    _gtMaxLimit: "对不起,您的已选项已经达到了15项.请减少已选项,再继续选择",
    _gtYourSelected: { cat: "您选择的职位类别是：", ind: "选择一个题库",ind2: "设置题量和分数，每题至少1分",ind3: "",ind4: "",ind6: "", loc: "您选择的试题属性是",ind5: "设置题量和分数，每题至少1分", comptype: "您选择的公司性质是：" },
    _gtPopupSelectorHeader: { cat: "职位类别", ind: "请选择题库",ind2: "请选择题型",ind3: "请选择副知识点",ind4: "请选择知识分类", loc: "请选择行业",ind5: "请选择题型",ind6: "请选择适用对象", comptype: "公司性质" },
    _oneColumnLimit: { cat: 12, loc: 11 }, //二级类别显示成一列的最大条目数，超过这个数就要显示成两列
    _delay: 10,
    _style: {
        className: {
            levelOne: { "cat": "lay_wl", "loc": "lay_wls", "ind": "lay_wll","ind2": "lay_wll","ind3": "lay_wll","ind4": "lay_wll","ind5": "lay_wll","ind6": "lay_wll", "comptype": "lay_wms" },
            //二级类别一列显示的样式
            levelTwo1: { "cat": "lay_wm", "loc": "lay_ws lm", "ind": "","ind2": "","ind3": "","ind4": "","ind5": "","ind6": "", "comptype": "" },
            //二级类别两列显示的样式
            levelTwo2: { "cat": "lay_wl2", "loc": "lay_ws lm", "ind": "","ind2": "","ind3": "","ind4": "","ind5": "","ind6": "", "comptype": "" },
            //二级类别一列显示的样式
            levelThree1: { "cat": "lay_wm", "loc": "lay_ws", "ind": "","ind2": "","ind3": "","ind4": "","ind5": "","ind6": "", "comptype": "" },
            //3级类别两列显示的样式
            levelThree2: { "cat": "lay_wl2", "loc": "lay_ws", "ind": "","ind2": "","ind3": "","ind4": "","ind5": "","ind6": "", "comptype": "" }
        },
        left: 0,
        top: 0,
        width: { "cat": 594, "loc": 466, "ind": 346,"ind2": 346,"ind3": 346,"ind4": 346,"ind5": 346,"ind6": 346, "comptype": 360 },
        height: { "cat": 517, "loc": 500, "ind": 450,"ind2": 450,"ind3": 450,"ind4": 450,"ind5": 450,"ind6": 450, "comptype": 173 },
        lineHeight: 20,
        topBottomMargin: 17,
        offset: {
            levelOne: { X: 0, Y: 20, "ind": { X: 100, Y: 20 },"ind2": { X: 100, Y: 20 },"ind3": { X: 100, Y: 20 },"ind4": { X: 100, Y: 20 },"ind5": { X: 100, Y: 20 },"ind6": { X: 100, Y: 20 }, "loc": { X: 250, Y: 20} },
            levelTwo: { "ind": { X: 104, Y: 0 },"ind2": { X: 104, Y: 0 },"ind3": { X: 104, Y: 0 },"ind4": { X: 104, Y: 0 },"ind5": { X: 104, Y: 0 },"ind6": { X: 104, Y: 0 }, "loc": { X: 190, Y: 10} }
        }
    },
    allItems: {
        cat: function() { return getCat(); },
        ind: function() { return getInd(); },
        ind2: function() { return getInd2(); },
        ind5: function() { return getInd5(); },
        ind3: function() { return getInd4('7'); },
        ind4: function() { return getInd4('8'); },
        ind6: function() { return getInd4('9'); },
...
jQuery(document).ready(function() {
    ...
    jQuery("#txtInd3").click(function() { PopupSelector.popup("ind3", this); });//副知识点
    jQuery("#txtInd4").click(function() {jQuery("#btnCheckAll").show(); PopupSelector.popup("ind4", this); });//认知水平
    jQuery("#txtInd6").click(function() {jQuery("#btnCheckAll").show(); PopupSelector.popup("ind6", this); });//职称
    jQuery("#txtIndDrop").click(function() {
    	if($("#questType_2").val() == ''){
	    	jQuery("#btnCheckAll").show(); PopupSelector.popup("ind", jQuery("#txtInd")[0]); 
		}else {
    ...
    jQuery("#txtIndDrop_hand").click(function() {jQuery("#btnCheckAll").show(); PopupSelector.popup("ind", jQuery("#txtInd")[0]); });
    jQuery("#txtIndDrop4_hand").click(function() {
    	jQuery("#btnCheckAll").show(); 
		PopupSelector.popup("ind4", jQuery("#txtInd")[0]); 
    });
    ...
    jQuery("#txtIndDrop2").click(function() {
    	if($("#question_label_ids").val() == ''){
	    	jQuery("#btnCheckAll").hide();
	    	jQuery("#mask").height(jQuery(document).height()).show();
	    	PopupSelector.popup("ind2", jQuery("#txtInd2")[0]);
		}else {
    ...
    jQuery("#btnCheckAll").click(function() { PopupSelector.checkAll(); });
...
function getInd() {
	...
	$.ajax({
    	type:"post",
    	url: ctxJS +"/exam/examQuesList.do?method=quesTypes",
    	dataType:"json",
    	async:false,
    	cache:true,
...
function getInd2() {
	...
	$.ajax({
    	type:"post",
    	data:"questType_2="+$("#questType_2").val()+"&questCognize_8_2="+$("#questCognize_8_2").val()+"&questTitle_9="+$("#questTitle_9").val(),
    	url: ctxJS +"/exam/examQuesList.do?method=quesLable&flag=true",
    	dataType:"json",
    	async:false,
    	cache:true,
    	success: function(rdata) {
...
function getInd5() {
	...
	$.ajax( {
		type : "post",
		data : "questType_2=" + $("#questType_2").val() + "&renderSelIds="
				+ $("#renderSelIds").val() + "&questCognize_8_2="
				+ $("#questCognize_8_2").val()+ "&questTitle_9="
				+ $("#questTitle_9").val(),
		url : ctxJS +"/exam/examQuesList.do?method=quesLable",
		dataType : "json",
		async : false,
		cache : true,
		success : function(b) {
...
function getInd4(propType) {
	...
	$.ajax({
    	type:"post",
    	url:ctxJS +"/exam/examQuesList.do?method=quesPorp",
...
function countQues(flag) {
	...
	$.ajax({
    	type:"post",
    	data:"questType_2="+$("#questType_2").val()+"&questCognize_8_2="+$("#questCognize_8_2").val()+"&questTitle_9="+$("#questTitle_9").val()+"&renderSelIds="+$("#renderSelIds").val()+"&question_label_ids="+$("#question_label_ids").val()+"&flag="+flag,
    	url:ctxJS +"/exam/examQuesList.do?method=countQues",
    	async:false,
    	cache:true,
    	success: function(rdata) {
			rd = rdata;
...
//{"id":"5400","subItems":[],"name":"甲状腺功能亢进症"}
function getLoc() {
	var a = {};
	$.ajax( {
		type : "post",
		url : ctxJS +"/exam/examQuesList.do?method=getLoc",
		dataType : "json",
		async : false,
		//cache : true,
		success : function(b) {
