...
oldChildStr = "<span id='c_span${t.index}'><table border='0' cellspacing='1' cellpadding='3' width='99%' class='gridtable'><tr><td class='row_tip'>试题类型：</td><td>多选题(A" + eval(ql/3 + 2) + "-2)</td></tr><tr><td class='row_tip'>试题内容：</td><td><textarea name='_content${t.index}' id='_content${t.index}' disabled='disabled' style='width: 92%' rows='2'>${child.content}</textarea></td></tr><tr><td class='row_tip'>试题分析：</td><td><textarea style='width: 92%' id='_analyse${t.index}' name='_analyse${t.index}' disabled='disabled' rows='2' >${child.analyse}</textarea></td></tr><tr><td class='row_tip'>候选项：</td><td>";
<c:forEach items="${child.questionKeyList}" var="key" varStatus="k">
oldChildStr += "<input type='checkbox' name='_xx${t.index}' <c:if test='${key.isnot_true eq 1}'>checked='checked'</c:if> disabled='disabled' />" + String.fromCharCode(${k.index}+65) + ".<input type='text' name='_xxnr${t.index}' id='_xxnr${t.index}' disabled='disabled' value='${key.content}'/><br/>";
xxStr += "${key.content}/${key.isnot_true}|";
</c:forEach>
oldChildStr += "</td></tr><tr><td class='row_tip' colspan='2'><input name='sub_questions' id='sub_questions' type='hidden' value='${child.question_label_id}|${child.content}|" + xxStr + "|${child.analyse}|' /><input type='button' class='but2' value='删除子试题' onclick='delQuestion("+strArray.length+")'/></td></tr></table></span>";
</c:when>
<c:otherwise>
oldChildStr = "<span id='c_span${t.index}'><table border='0' cellspacing='1' cellpadding='3' width='99%' class='gridtable'><tr><td class='row_tip'>试题类型：</td><td>单选题(A" + eval(ql/3 + 2) + "-1)</td></tr><tr><td class='row_tip'>试题内容：</td><td><textarea name='_content${t.index}' id='_content${t.index}' disabled='disabled' style='width: 92%' rows='2'>${child.content}</textarea></td></tr><tr><td class='row_tip'>试题分析：</td><td><textarea style='width: 92%' id='_analyse${t.index}' name='_analyse${t.index}' disabled='disabled' rows='2' >${child.analyse}</textarea></td></tr><tr><td class='row_tip'>候选项：</td><td>";
<c:forEach items="${child.questionKeyList}" var="key" varStatus="k">
oldChildStr += "<input type='radio' disabled='disabled' name='_xx${t.index}' <c:if test='${key.isnot_true eq 1}'>checked='checked'</c:if> id='_xx${t.index}' value='#2' />" + String.fromCharCode(${k.index}+65) + ".<input type='text' name='_xxnr${t.index}' id='_xxnr${t.index}' disabled='disabled' value='${key.content}'/><br/>"
xxStr += "${key.content}/${key.isnot_true}|";
</c:forEach>
oldChildStr += "</td></tr><tr><td class='row_tip' colspan='2'><input name='sub_questions' id='sub_questions' type='hidden' value='${child.question_label_id}|${child.content}|" + xxStr + "|${child.analyse}|' /><input type='button' class='but2' value='删除子试题' onclick='delQuestion("+strArray.length+")'/></td></tr></table></span>";
</c:otherwise>
