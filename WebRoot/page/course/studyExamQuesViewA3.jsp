...
<table border="0" cellspacing="1" cellpadding="0" width="100%" class="gridtable">
	...
	<tr>
		<td class="row_tip"  colspan="4" height="25">
			子试题
		</td>
	</tr>
	<tr>
		...
		<!-- child -->
			<table width="100%" border="0" cellpadding="3" cellspacing="0" class="gridtable">
				<tr>
					<td class="row_tip">试题题型: </td>
					<td colspan="3">
					<c:choose>
						<c:when test="${child.question_label_id eq 4}">单选题(A3-1/A4-1)</c:when>
						<c:when test="${child.question_label_id eq 5}">多选题(A3-2/A4-2)</c:when>
					</c:choose>
					</td>
				</tr>
				<tr>
					<td class="row_tip">试题内容:</td>
					<td colspan="3">
						<c:out value="${child.content}" escapeXml="false"/>
					</td>
				</tr>		
				<tr>
					<td class="row_tip">选项:</td>
					<td colspan="3">
					<c:forEach var="key" items="${child.questionKeyList}" varStatus="status">
					<c:choose>
						<c:when test="${child.question_label_id eq 4}">
							<input type="radio" disabled="disabled" <c:if test="${key.isnot_true eq 1}"> checked </c:if> name="key${status.index}${child.id}">${key.content}<br>
						</c:when>
						<c:when test="${child.question_label_id eq 5}">
							<input type="checkbox" disabled="disabled" <c:if test="${key.isnot_true eq 1}"> checked </c:if> name="key">${key.content}<br>
						</c:when>
					</c:choose>								
					</c:forEach>
					</td>
				...
				<tr>
					...
					<td colspan="3">
						<c:out value="${child.analyse}" escapeXml="false"/>
					</td>
				</tr>
			</table>
			<hr>
