...
<table border="0" cellspacing="1" cellpadding="0" width="100%" class="gridtable">
	...
	<tr>
		<td class="row_tip">
			候选项<br><br>
			<a href="javascript:addrowDuoX1('key_a')" >增加候选项</a><br><br>
			...
			<c:forEach var="qk" items="${quest.questionKeyList}" varStatus="i">
				<tr>
					<td width="6%">
						...
						<script type="text/javascript">document.write(String.fromCharCode(i++));</script>:<input type="checkbox" <c:if test="${qk.isnot_true eq 1}">checked</c:if> value="<c:out value="${i.index+1}"/>" name="answer_key" id="answer_key"/>
					</td>
					<td>
						<textarea name="answer_content" id="answer_content<c:out value="${i.index+1}"/>" style="width: 91.5%" rows="3">${qk.content}</textarea>
						<a href="javascript:dialog('adv_edit.jsp',600,430,'answer_content<c:out value="${i.index+1}"/>');">高级编辑</a>
					</td>
				</tr>
			</c:forEach>
			</table>			
