		...
		<div>
			<div id="fxbg_center">
				...
				<div>
					<ul style="list-style:none;margin:0;">
						<li style="float:left;padding-left:30px;">
							<strong>试卷名称：</strong>${paper.name}
						</li>
						<li style="float:left;padding-left:30px;">
							<strong>组卷方式：</strong>
							<c:if test="${paper.paper_mode == 1}">快捷策略1</c:if>
							<c:if test="${paper.paper_mode == 5}">快捷策略2</c:if>
							<c:if test="${paper.paper_mode == 2}">精细策略</c:if>
							<c:if test="${paper.paper_mode == 3}">手工组卷</c:if>
							<c:if test="${paper.paper_mode == 4}">卷中卷</c:if>
						</li>
						<li style="float:left;padding-left:30px;">
							<strong>试卷分数：</strong>${paper.paper_score}
						</li>
						<li style="float:left;padding-left:30px;">
							<strong>试题数量：</strong>${paper.question_num}
						</li>
					</ul>
					<br />
				</div>
				...
				<div id="kspj_main_bottom" style="text-align: center;">
					<input type="button" class="btn_03s" value="关 闭" onclick="window.parent.closeWin();" />
				</div>
			</div>
		</div>
