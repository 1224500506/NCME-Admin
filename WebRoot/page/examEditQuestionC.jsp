...
<table border="0" cellspacing="1" cellpadding="0" width="100%" class="gridtable">
	...
	<tr>
		<td class="row_tip">
			<font class="red_star"">*</font>标准答案：
		</td>
		<td colspan="5">
			<c:forEach var="qk" items="${quest.questionKeyList}">	
			<input type="hidden" value="${qk.id}" name="questionkey_id"/>
			<textarea name="answer_content" id="answer_content" class="editTextarea"style="width: 92%" rows="3" elname="标准答案" pattern="string" required="1">${qk.content}</textarea>
			<a href="javascript:dialog('adv_edit.jsp',600,430,'answer_content');">高级编辑</a>
			</c:forEach>
		</td>
	</tr>
	<tr>
...
function xaction(){
	...
	var question_answer_content = document.fm1.answer_content.value.getBytes();
	...
	if(question_answer_content ==0 || question_answer_content>2000){
		alert('标准答案不能为空且不能大于2000个字符或1000个汉字!');
