...
<table border="0" cellspacing="1" cellpadding="0" width="100%" class="gridtable" align="center">
	<tr>
	...
	<tr>
		...
		<td>
			...
			<select id="question_label_id" name="question_label_id" disabled>
		...
		<td>
			<select id="isnot_multimedia" name="isnot_multimedia">
				<option value="0" <c:if test="${quest.isnot_multimedia eq 0}">selected="selected"</c:if>>否</option>
				<option value="1" <c:if test="${quest.isnot_multimedia eq 1}">selected="selected"</c:if>>是</option>
			</select>
		</td>
	</tr>
	<tr>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" id="child_questions" class="gridtable">
		<tr>
					<td align="center" class="row_tip" height="25px">子试题信息</td>
				</tr>			
				<tr>
					<td class="td1">
...
var win1 = new ChildWin(${quest.question_label_id});
var oldChildStr= "";
var xxStr = "";
...
function xaction(){
	...
	if(document.getElementById('sub_questions')==null){
		alert('请添加子试题!');
		return;
	}
...
var ql = document.getElementById("question_label_id").value;

<c:forEach var="child" items="${quest.childQuestionList}" varStatus="t">
	<c:choose>
		<c:when test="${child.question_label_id eq 5}">
			var child_content = replaceHtml('${child.content}');
			var child_analyse = replaceHtml('${child.analyse}');
			oldChildStr = "<span id='c_span${t.index}'><table border='0' cellspacing='1' cellpadding='3' width='99%' class='gridtable'><tr><td class='row_tip'>试题类型：</td><td>多选题(A" + eval(ql/3 + 2) + "-2)</td></tr><tr><td class='row_tip'>试题内容：</td><td><textarea name='_content${t.index}' id='_content${t.index}' disabled='disabled' style='width: 92%' rows='2'>"+child_content+"</textarea></td></tr><tr><td class='row_tip'>试题分析：</td><td><textarea style='width: 92%' id='_analyse${t.index}' name='_analyse${t.index}' disabled='disabled' rows='2' >"+child_analyse+"</textarea></td></tr><tr><td class='row_tip'>候选项：</td><td>";
				<c:forEach items="${child.questionKeyList}" var="key" varStatus="k">
					var key_content = replaceHtml('${key.content}');
					oldChildStr += "<input type='checkbox' name='_xx${t.index}' <c:if test='${key.isnot_true eq 1}'>checked='checked'</c:if> disabled='disabled' />" + String.fromCharCode(${k.index}+65) + ".<input type='text' name='_xxnr${t.index}' id='_xxnr${t.index}' disabled='disabled' value='"+key_content+"'/><br/>";
					xxStr += key_content+"//${key.isnot_true}|";
				</c:forEach>
			oldChildStr += "</td></tr><tr><td class='row_tip' colspan='2'><input name='sub_questions' id='sub_questions' type='hidden' value='${child.question_label_id}|"+child_content+"|" + xxStr + "|"+child_analyse+"|' /><input type='button' class='but2' value='删除子试题' onclick='delQuestion("+strArray.length+")'/></td></tr></table></span>";
		</c:when>
		
		<c:otherwise>
			var child_content = replaceHtml('${child.content}');
			var child_analyse = replaceHtml('${child.analyse}');
			oldChildStr = "<span id='c_span${t.index}'><table border='0' cellspacing='1' cellpadding='3' width='99%' class='gridtable'><tr><td class='row_tip'>试题类型：</td><td>单选题(A" + eval(ql/3 + 2) + "-1)</td></tr><tr><td class='row_tip'>试题内容：</td><td><textarea name='_content${t.index}' id='_content${t.index}' disabled='disabled' style='width: 92%' rows='2'>"+child_content+"</textarea></td></tr><tr><td class='row_tip'>试题分析：</td><td><textarea style='width: 92%' id='_analyse${t.index}' name='_analyse${t.index}' disabled='disabled' rows='2' >"+child_analyse+"</textarea></td></tr><tr><td class='row_tip'>候选项：</td><td>";
				<c:forEach items="${child.questionKeyList}" var="key" varStatus="k">
				var key_content = replaceHtml('${key.content}');	
				oldChildStr += "<input type='radio' disabled='disabled' name='_xx${t.index}' <c:if test='${key.isnot_true eq 1}'>checked='checked'</c:if> id='_xx${t.index}' value='#2' />" + String.fromCharCode(${k.index}+65) + ".<input type='text' name='_xxnr${t.index}' id='_xxnr${t.index}' disabled='disabled' value='"+key_content+"'/><br/>"
					xxStr += key_content+"//${key.isnot_true}|";
				</c:forEach>
			oldChildStr += "</td></tr><tr><td class='row_tip' colspan='2'><input name='sub_questions' id='sub_questions' type='hidden' value='${child.question_label_id}|"+child_content+"|" + xxStr + "|"+child_analyse+"|' /><input type='button' class='but2' value='删除子试题' onclick='delQuestion("+strArray.length+")'/></td></tr></table></span>";
		</c:otherwise>
	</c:choose>
	strArray[strArray.length]=[oldChildStr,0];
	oldChildStr = "";
	xxStr = "";
</c:forEach>
child_index=${fn:length(quest.childQuestionList)};
closeLayer();
