...
<table border="0" cellspacing="1" cellpadding="0" width="100%" class="gridtable">
	...
	<tr>
		<td class="row_tip">
			<font class="red_star"">*</font>候选项：
		</td>
		<td colspan="5">
			<c:forEach var="qk" items="${quest.questionKeyList}">
				<input type="hidden" value="${qk.id}" name="questionkey_id"/>
				<input type="radio" value="1" name="answer_key" id="answer_key" <c:if test="${qk.isnot_true eq 1}">checked="checked"</c:if> />正确 &nbsp; 
				<input type="radio" value="2" name="answer_key" id="answer_key" <c:if test="${qk.isnot_true eq 0}">checked="checked"</c:if> />错误
			</c:forEach>
		</td>
	</tr>
	<tr>
