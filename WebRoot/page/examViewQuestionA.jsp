			...
			<c:forEach var="qk" items="${quest.questionKeyList}" varStatus="i">
				<tr>
					<td width="6%">
						...
						<input type="radio" <c:if test="${qk.isnot_true eq 1}">checked</c:if> value="${i.index+1}" name="answer_key" id="answer_key" />
						</c:when>
						<c:otherwise>
						<input type="radio" value="${i.index+1}" name="answer_key" id="answer_key" disabled="disabled"/>
						</c:otherwise>
						</c:choose>
						${qk.content}
					</td>
