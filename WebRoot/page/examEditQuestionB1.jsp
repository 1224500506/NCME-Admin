...
<table width="100%" border="0" cellpadding="0" cellspacing="0" id="child_questions" class="dataintable">
	<tr>
		<th class="stcx_form_title" colspan="2">子试题信息</th>
	</tr>
	<tr>
		<td class="tdTitle" width="80%"><b>试题内容</b></td>
		<td class="tdTitle" width="20%"><b>操作</b></td>
	</tr>
	<c:forEach var="child" items="${quest.childQuestionList}">
	<tr>
		<td class="td1">
		<%request.setAttribute("yin", "\""); %>
		<c:out value="${fn:substring(child.content,0,30)}" escapeXml="false"/>
		<c:if test="${fn:length(child.content)>30}">...</c:if>
		<input type="hidden" value="${child.questionKeyList[0].content}" name="child_answer" />
		
		<c:set var ="yy" value="${fn:replace(child.analyse,yin,'“')}"></c:set>		
		<input type="hidden" value="${yy}" name="child_analyse" />
		
		<c:set var ="cc" value="${fn:replace(child.content,yin,'“')}"></c:set>		
		<input type="hidden" value="${cc}" name="child_content" id="child_content" />
		</td>		
		<td class="td1"><a href="#" onclick="view_childquestion(event,child_questions);">编辑</a> | <a href="#" onclick="deleteRow(event,child_questions,'确定要删除吗？');">删除</a></td>
	</tr>						
	</c:forEach>
</table>
</jodd:form>
