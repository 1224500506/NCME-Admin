...
<%-- 				<a href="${ctx}/expert/ExpertManage.do?mode=add" class="fl" style="width:95px;margin-right:15px;">添加专家</a> --%>
		...
		<div class="clear"></div> 
			<display:table name="extList" requestURI="${ctx}/expert/ExpertManage.do?mode=listms" id="p" class="mt10 cas_table" size="${totalCount}" pagesize="20"  sort="list" defaultsort="1" defaultorder="descending">
				...
				<display:column title="任职状态" style="width:5%;">
 				...
 				<display:column title="操作" style="width:15%;">
					...
					<a href="javascript:doLock(${p.id},${p.lockState});" >
						<c:if test="${p.lockState==1}">绑定</c:if>
						<c:if test="${p.lockState==0}">解绑</c:if>
					</a>
 				</display:column>
			</display:table>
