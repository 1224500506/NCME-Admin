...
<div>
	...
	<div class="center">
		...
		<h2 style="font-size:16px; font-weight:600;">查看【导航栏 &gt; 名师 】</h2>
		
		<div class="tk_center01" style="min-height:160px;">
		<form id="sfrm" action="${ctx}/expert/ExpertManage.do?mode=listView" method="POST">
			<div class="tk_zuo">
	...
	<div class="tk_center01">
		...
		<div class="clear"></div> 
			<display:table name="extList" requestURI="${ctx}/expert/ExpertManage.do?mode=listView" id="p" class="mt10 cas_table" size="${totalCount}" pagesize="20"  sort="list" defaultsort="1" defaultorder="descending">
				...
				<display:column title="职务" sortable="true" property="officeNames" style="width:8%;"></display:column>
 				...
 				<display:column title="单位职务" sortable="true" style="width:8%;" property="workUnit_office">
				...
				<display:column title="任职状态" style="width:5%;">
 				...
 				<input type="hidden" id="lockState" value="${p.lockState}">
					<a href="${ctx}/expert/ExpertManage.do?mode=detailView&id=${p.id}" >查看</a>
...
function doLock(id,lockState){
	...
	var url = '${ctx}/expert/ExpertManage.do?mode=lockView';
	if(id!=''){
...
function doBatchDelProp() {
	...
	if(confirm("确认解绑吗?")){
		...
		$.post("${ctx}/expert/ExpertManage.do?mode=lockViewAll", p,
  			function(data){
  				if(data != "fail"){
  					alert("所选项解绑成功!");
  				}
