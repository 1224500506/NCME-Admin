...
<%@include file="/commons/taglibs.jsp"%>
	...
	<%-- <link rel="stylesheet" href="${ctxAll}/peixun_page/js/lib/layui/css/layui.css" /> --%>
...
<!-- 			</div> -->
				<div>
	               <button type="button" name="tijiao" id="tijiao" class="denglubtn">提&nbsp;&nbsp;&nbsp;交</button>
	               <button type="button" name="quxiao" id="quxiao" class="denglubtn">取&nbsp;&nbsp;&nbsp;消</button>
	            </div>
		</div>
	</div>
</div>
...
// 		 window.localStorage.getItem("CDCode");
	...
	function initPropList(_title,_ajaxurl, _initType, _initId, _selectLvl, _checkLvl, _kuangcode, _kuang, nowAuthorType){
		...
		if(_initType > 0 && _initType < 6){
			...
			//if(nowAuthorType == '5'){
			//	$('.checkAllDiv').css('display','inline-none');
			//}else{
			...
			$.ajax({
			    ...
			    success: function(result){
				   if(result != ''){
						...
						$(selarray).each(function(key, val){
							if(nowAuthorType == '5'){
								if (val != "") newnarray.push('<em class="">省/直辖市 >' + val + '</em>');
							}else{
								if (val != "") newnarray.push('<em class="delem">省/直辖市 >' + val + '</em>');
							}
						});
						$('.xs_kuang').html(newnarray.toString().replace(dotexp, "<br>") +"<br>");
						oldPropname = $('.xs_kuang').html();
						oldPropcode = $(_kuangcode).val();
						$('.delem').click(function(){
							delem($(this));
						});
				   }
	...
	*/
	...
	function initPropWndProp(){
		...
		{
			var aaaa = oldPropcode;
			$('.xs_ul input[type="checkbox"]').off('click');
			$('.xs_ul input[type="checkbox"]').click(function(){
			
				if(typeof(oldPropname) == "undefined"){
					oldPropname = '';
				}
				if(typeof(oldPropcode) == "undefined"){
					oldPropcode = '';
				}
				...
				if ($(this).prop('checked')){
					...
					var selstr = $('.xs_kuangcode').text();
					//var selstr = oldPropcode + $('.xs_kuangcode').text();
					var selarray = selstr.split(",");
					var idx = selarray.indexOf(id);
					var newarray = new Array();
					$(selarray).each(function(key, val){
					...
					//if(ahthorType == '6'){//当为添加授权时则实时更新地区code
						$('.xs_kuangcode').text(newarray.toString());
					//}
					//$('.xs_kuangcode').text(newarray.toString());
	...
	function delem(obj){
		...
		if(typeof(nowPropcode) == "undefined"){
			nowPropcode = '';
		}
		//delete code
		var deletecode = "";
		selstr = nowPropcode + $('.xs_kuangcode').text();
		selarray = selstr.split(",");
		newarray = new Array();
		$(selarray).each(function(key, val){
...
//window control code
	...
	function checkAllDiv(){
		...
		if($("#checkall").attr("checked")=="checked"){
			var checkAllArrId = [];
			var checkAllArrEm = [];
			$('.xs_kuang').html(oldPropname);
			$('.xs_kuangcode').text(oldPropcode);
			nowPropcode = oldPropcode;
			//全选
			for(i=0;i<CheckBox.length;i++){
				CheckBox[i].checked=true;
				checkAllArrId.push(CheckBoxP[i].id);
				//var propname = '<br><em class="delem">省/直辖市 >';
			...
			$('.xs_kuangcode').append(checkAllArrId.toString());
