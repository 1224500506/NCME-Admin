...
<div class="center">
	...
	<div class="tiaojian" style="min-height:40px;">
		<%--?pageNum=3&parentId=${parentId}&typeId=${typeId}&qualityId=${qualityId}&parentName=${parentName}--%>
		...
		<div class="fr cas_anniu">
			...
			<a href="###" class="fl" style="width:70px;" onclick="goThreeLevelBefore('${parentId}','${beforeParentName}');">返回</a>
		</div>
		<div class="clear"></div>
	</div>
	</form>
...
<div class="center" style="">
	...
	<!-- 内容 -->
		...
		<display:table name="list" requestURI="${ctx}/qualityManage/guide.do" id ="p" class="mt10 table" excludedParams="method" size="${page.fullListSize}" pagesize="${page.objectsPerPage}">
			...
			<display:column title="学习地图二级"  style="width:35%;">${p.name}</display:column>	
			<display:column title="已关联主题"  style="width:35%;">
				<c:forEach items="${p.subjectPropList}" var="item">
		    		${item.name}&nbsp;
		  	 	</c:forEach>
			</display:column>	
			<display:column title = "操作" style ="width:25%;">
				<a href="###" onclick="goThreeLevel('${parentId}','${p.id}','${param.qualityId}','${parentName}','${p.name}');">三级</a>
			</display:column>
...
function goThreeLevelBefore(parentId,parentName) {
   var pAllName = parentName  ;
   pAllName = encodeURI(pAllName) ;
   window.location.href = "${ctx}/qualityManage/guide.do?pageNum=2&parentId=" + parentId + "&parentName=" + pAllName ;
}
function goThreeLevel(parentId,pId,qualityId,parentName,pName) {
// 	localStorage.clear();
	localStorage.removeItem("bbb");
	localStorage.setItem("bbb", qualityId);
   var pAllName = parentName + " > " + pName ;
   pAllName = encodeURI(pAllName) ;
   window.location.href = "${ctx}/qualityManage/guide.do?pageNum=4&parentId=" + parentId + "&qualityId=" + pId + "&beforeQualityId=" + qualityId + "&parentName=" + pAllName ;
}
$(function(){
