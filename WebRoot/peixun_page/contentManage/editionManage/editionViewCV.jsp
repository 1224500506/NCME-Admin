...
<body>
<div class="clear"></div>
<form name="queryForm" id = "queryForm" action="${ctx}/contentManage/editionManage.do?mode=viewCV&id=${id}" method="post">
<!-- 查询条件 -->
<div class="center">
	<h2 style="font-size:16px; font-weight:600;">查看【首页  &gt; 课程】绑定内容</h2>
...
<div class="center" style="">
	<div class="center01">
		<div class="fr cas_anniu">
			<a href="javascript:void(0)" onclick="unbindCV('${id}')" class="fl" style="width:180px;">解除绑定</a>&nbsp;&nbsp;&nbsp;&nbsp;
		</div>
		<div class="clear"></div>
		<display:table name="list" requestURI="${ctx}/contentManage/editionManage.do?mode=viewCV" id="p" class="mt10 table" pagesize="15">			
...
$(function(){
	...
	function setorderNum(){
		var url = "${ctx}/contentManage/editionManage.do?mode=setorderCV";
		var params = "orderstr=";
		var isblank = false;
			...
			$.ajax({
			    ...
			    success: function(result){
				   if(result == 'success'){
				   		...
				   		alert("修改不成功！请重新输入排序号！");
				   }
			    }
			});
		}else{
			alert("排序中有重复的序号，请重新输入！");
			return false;
		}
	...
	function unbindCV(id){
		...
		if(confirm("确定解除绑定?")) {
			...
			$.post("${ctx}/contentManage/editionManage.do?mode=unbindCV", p,
					function(data){
						if(data >0){
							alert("解绑成功!");
						}
