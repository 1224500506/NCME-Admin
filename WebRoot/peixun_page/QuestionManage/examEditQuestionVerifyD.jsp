	...
	<form id="fm1" name="fm1" action="${ctx}/questionManage/updateQuestion.do" method="post">	
		...
		<ul>
			...
			<li>
				...
				<div class="shitin_xinzeng01">
					<p class="fl"><span class="fr">试题内容：</span><em class="fr">*</em></p>
					<textarea name="content" cols="" rows="" class="fl tki_bianji01"  style="width:580px;" readonly="readonly" id="content">${quest.content}</textarea>
				</div>
				<div class="clear"></div>
			</li>
			<li>
					...
					<div class="fl semiOr" id="key_a">						
						...
						<c:forEach var="qk" items="${quest.questionKeyList}" varStatus="i">
							<div class="mt10 tik_danxuan">
								<c:if test="${questionLabel != 13 }">
									<p class="fl">
										<input type="radio" <c:if test="${qk.isnot_true eq 1}">checked</c:if> value="${i.index+1}" name="answer_key" id="answer_key" disabled class="fl"/>
										<em class="fl"><script type="text/javascript">document.write(String.fromCharCode(i++));</script></em>
									</p>
								</c:if>
								<c:if test="${questionLabel == 13 }">
									<p class="fl" style="width: 55px;">
										<input type="radio" <c:if test="${qk.isnot_true eq 1}">checked</c:if> value="1" name="answer_key" id="answer_key" disabled class="fl"/>正确
									</p>
									<p class="fl" style="width: 55px;">						
										<input type="radio" <c:if test="${qk.isnot_true eq 0}">checked</c:if> value="2" name="answer_key" id="answer_key" disabled class="fl"/>错误
									</p> 							
								</c:if>
							</div>
							<div class="clear"></div>
