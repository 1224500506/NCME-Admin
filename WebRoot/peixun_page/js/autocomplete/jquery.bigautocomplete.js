			...
			$this.keyup(function(event) {
				...
				if(!isFunctionalKey  ){
					...
					if(data != null && $.isArray(data) ){
						...
						$.post(url,{"sname":keyword_},function(result){
							//非默认返回格式处理 lyj
							
							var datalist = result.list;
							
							MyAutoData = new Array();
							
							
							MyAutoData.splice(0);
							$(datalist).each(function(key, value){
								MyAutoData.push(addData(value.name,value.id));
							});
						
							/////
							
							//makeContAndShow(result.data)
							makeContAndShow(MyAutoData)//lyj
						},"json")
					}
