	...
	<div class="center01">
		<div class="tiaojian">
			<p class="fl"  style="margin:0 20px 20px 0;">
				...
				<input type="text" class="auth_site" value='<c:forEach items="${View.siteList}" var="site">${site.domainName};&nbsp;</c:forEach>' />
				<input type="hidden" id="siteIds" name="siteIds" value='<c:forEach items="${View.siteList}" var="site">${site.id},</c:forEach>'/>
			</p>
			<p class="fl">
			...
			<p class="fl" style="margin:0 20px 20px 0;">
				...
				<input type="text" id = "serialNumber" name = "serialNumber" value="${View.cVSetAuthorization.cvSetSerialNumber}" placeholder="请输入" />
			</p>
			<p class="fl">
			...
			<p class="fl">
					...
					<c:if test="${View.cVSetAuthorization.cvSetLevel==6}">
						<input type="text" id = "score" disabled="disabled" name = "score" onkeyup="this.value= this.value.replace(/[^\d.]/g, '');" value="${View.cVSetAuthorization.cvSetScore}" placeholder="请输入学分" style="margin-left:10px;width:110px;" />
					</c:if>
					<c:if test="${View.cVSetAuthorization.cvSetLevel!=6}">
						<input type="text" id = "score" name = "score" onkeyup="this.value= this.value.replace(/[^\d.]/g, '');" value="${View.cVSetAuthorization.cvSetScore}" placeholder="请输入学分" style="margin-left:10px;width:110px;" />
					</c:if>
			</p>
			<div class="clear"></div>
			<p class="fl" style="margin:0 20px 20px 0;">
				<span style="width:9em;text-align:right;"><em class="red_start">*</em> 项目价格：</span>
    			<input type="radio" id="costType" name="costType" value="0" <c:if test="${View.cVSetAuthorization.cvSetCostType== 0}">checked="checked"</c:if> style="margin-top:7px;float:left;" onClick="radiocheck();"><span style="color:#333;">免费</span>
    			<input type="radio" id="costType" name="costType" value="1" <c:if test="${View.cVSetAuthorization.cvSetCostType== 1}">checked="checked"</c:if> style="margin-top:7px;float:left;margin-left:15px;" onClick="radiocheck();"><span style="color:#333;">学习卡</span>
    			<input type="radio" id="costType" name="costType" value="2" <c:if test="${View.cVSetAuthorization.cvSetCostType== 2}">checked="checked"</c:if> style="margin-top:7px;float:left;margin-left:15px;" onClick="radiocheck();"><span style="color:#333;">收费</span>
    			<span class="fr" style="margin:0 5px;color:#333;" id="costUnit">元</span>
    			<input type="text" id = "cost" name = "cost" onkeyup="this.value= this.value.replace(/[^\d]/g, '');" value="${View.cost }" placeholder="请输入价格"  style="float:right;width:100px;height:25px;border:1px solid #ddddd;margin-left:15px;"/>
			...
			<p class="clear1" style="margin:0 20px 50px 0;">
				...
				<span class="fl" style="margin-right:30px;color:#333;"><input type="checkbox" class = "sign" id="sign2" value = "指南解读"/> 指南解读</span>
				<span class="fl" style="margin-right:30px;color:#333;"><input type="checkbox" class = "sign" id="sign4" value = "乡医培训"/> 乡医培训</span>
				<span class="fl" style="color:#333;"><input type="checkbox" class = "sign" id="sign3" value = "海外视野"/> 海外视野</span>
				<input type = "hidden" id = "signStr" name = "signStr" value = "${View.cVSetAuthorization.cvSetSign}"/>
			</p>
			<div class="clear"></div>
			...
			<p class="fl" style="margin:0 20px 20px 0;">
				...
				<input type="text" class="datainp xiangmuks" id = "start_date" name = "start_date"  data-name="start_date_submit" value="<fmt:formatDate value="${View.cVSetAuthorization.cvSetStartDate}" type="both" pattern="yyyy-MM-dd"/>" placeholder="yyyy-MM-dd" /> <span style="margin:0 5px;color:#333;">至</span>
				<input type="text" class="datainp xiangmujs" id = "end_date" name = "end_date" data-name="end_date_submit" value="<fmt:formatDate value="${View.cVSetAuthorization.cvSetEndDate}" type="both" pattern="yyyy-MM-dd"/>" placeholder="yyyy-MM-dd" />
			</p>
			<p class="fl" style="margin:0 20px 20px 0;">
				<c:if test = "${View.forma == 1}">
					<span style="width:13em;text-align:right;"><em class="red_start">*</em> 项目课程的学习间隔时间：</span>
					<input type="text" id = "break_days" name = "break_days" value = "${View.cVSetAuthorization.cvSetBreakDays}" onkeyup="this.value= this.value.replace(/[^\d]/g, '');" placeholder="" /> <span class="fl" style="margin:0 5px;color:#333;">天</span>
				</c:if>
				<c:if test = "${View.forma == 3}">
					<span style="width:13em;text-align:right;"><em class="red_start">*</em> 项目报名截止时间：</span>
					<input type="text" id = "cvSetRegistrationStop" name = "cvSetRegistrationStop" value = "0" onkeyup="this.value= this.value.replace(/[^\d]/g, '');" placeholder="" /> <span class="fl" style="margin:0 5px;color:#333;">天</span>
				</c:if>
			</p>
			<div class="clear"></div>
				...
				<c:forEach items = "${View.CVScheduleList}" var = "schedule">
					<li class="clear">
						<p>
							...
							<input type="text" class="datainp kechengks" id = "item_${schedule.id}_start" name = "item_${schedule.id}_start"  data-name="item_${schedule.id}_start_submit"   value="<fmt:formatDate value="${schedule.start_date}" pattern="yyyy-MM-dd"/>" placeholder="yyyy-MM-dd" /> <span style="margin:0 5px;color:#333;">至</span>
							<input type="text" class="datainp kechengjs" id = "item_${schedule.id}_end" name = "item_${schedule.id}_end" data-name="item_${schedule.id}_end_submit"  value="<fmt:formatDate value="${schedule.end_date}" pattern="yyyy-MM-dd" />" placeholder="yyyy-MM-dd" /> 
 ...
 function save()
{
	...
	*/
	...
	if(nowstatus != null && nowstatus == '5'){
		var end_date = $('#end_date').val();
		var endDate = new Date((end_date+" 23:23:59").replace(/-/g,"/"));
		var date = new Date();
		if(endDate < date){
			var layerStr = '<div class="tiaojian" style="min-height:40px;"><p class="fl" style="margin-bottom:20px;margin-left:20px;"><span style="margin-top:20px;font-size:large;color:red;">项目截止时间不可早于当前时间！若要修改，请下线项目后修改。</span></p></div>';
		    var win_w = "350px";
			var win_h = "200px";
			layer.open({
				type: 1,
