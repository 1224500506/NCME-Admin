...
<body>
<!-- 查询条件 -->
<form  id = "queryForm" action="${ctx}/CVSet/CVDistribute.do?method=releaseList" method="post">
<div class="center" style="width:98%;">
	<div class="tiaojian">
		<p class="fl">
			<span style="width: 12em;text-align:right;">学科：</span>
...
<div class="center" style="">
	<div class="content01">
		<ul class="cas1_xuanxing">
	        <li class="action">未发布</li>
	        <li id="cvsetAlreadyRelease" style="margin-left:30px;">已发布</li>
	    </ul>
	    	<div class="fr cas_anniu">
				<a href="javascript:cvsRelease(-1);" class="fl" style="width:80px;margin-left:10px;margin-bottom:10px;margin-top:-60px;">发布</a>
			</div>
			<display:table name="CVSet" requestURI="${ctx}/CVSet/CVDistribute.do" id="CVS" class="mt10 table" pagesize="15">
					...
					<display:column title="项目状态" style="width:5%;">
						...
						<c:if test="${CVS.status==5}">
							<c:set var="nowTime" value="<%=System.currentTimeMillis()%>"></c:set>
							<c:choose>
							<c:when test="${nowTime-CVS.endTime>= 0}">
							<span class="STYLE1">已下线</span>
							</c:when>
							<c:otherwise>
							<span class="STYLE2">已发布</span>
							</c:otherwise>
							</c:choose>
						</c:if>
					...
					<display:column title="操作" style="width:6%;">						
						...
						<c:if test="${CVS.status !=4}">
							<a href="javascript:cvsRelease(${CVS.id});">发布</a>
						</c:if>													 
					</display:column>
		</display:table>
	</div>
...
//选择目录弹出框
		...
		$('#cvsetAlreadyRelease').click(function () {
...
//授权---项目发布--taoLiang
function cvsRelease(cvSetId){

	var params = 'method=cvSetReleaseAndOffline&cvSetId='+cvSetId+'&flag=5'
	//判断是否为批量
	if(cvSetId == -1){
		...
		params = 'method=cvSetReleaseAndOffline&cvSetIds='+selectedIds+'&flag=5'
	}
	if (!confirm("确定发布？发布后，学员可查看并学习该项目。")) return;
	$.ajax({
		type:'post',
