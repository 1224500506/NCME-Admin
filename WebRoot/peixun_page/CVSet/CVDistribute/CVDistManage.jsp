...
<body>
<!-- 查询条件 -->
<form  id = "queryForm" action="${ctx}/CVSet/CVDistribute.do?method=list" method="post">
...
<div class="center" style="">
	<div class="content01">
		<ul class="cas1_xuanxing">
	        <li class="action">项目列表</li>
	        <li id="authorinfo" style="margin-left:30px;">授权信息列表</li>
	    </ul>
			<display:table name="CVSet" requestURI="${ctx}/CVSet/CVDistribute.do" id="CVS" class="mt10 table" pagesize="15">
					...
					<display:column title="操作" style="width:6%;">						
						...
						<c:if test="${CVS.status !=4}">
							<a href="javascript:distribute(${CVS.id});">授权</a>
						</c:if>													 
					</display:column>
		</display:table>
	</div>
...
function distribute(CVSid)
{
	$.ajax({
		...
		data:'method=checkAuthorization&cvSetId='+CVSid,
		dataType:'JSON',
		success:function(result){
			if(result.flag == 0){
				alert("该项目的授权地区已覆盖全国范围，没有可授权区域！");
