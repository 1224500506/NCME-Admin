...
<!-- 查询条件 -->
<div class="center">
	<form action="${ctx}/examManage/examPaperDetail.do" id="fm" name="fm" method="post">
	<div class="tiaojian" style="min-height:40px;">
		<p class="fl">
			<span>选择目录：</span>	
			<input type="hidden" id="curTypeId" name ="type_Ids" value="${typeId}"/>
		...
		<p class="fl" style="margin-left:60px;">
			...
			<div class="fl select" >
				...
				<ul class="fl list" style="display:none;">
					<select name="paperMode" style="display:none;">
						<option value="">请选择</option>
						<option value="1"<c:if test="${paperMode==1}"> selected</c:if>>快捷策略</option>						
						<option value="5"<c:if test="${paperMode==5}"> selected</c:if>>精细策略</option>
...
<div class="center" style="">
	<div class="center01">
		...
		<form action="" id="formB" name="formB" method="post">
			<display:table id="item" list="${List}" requestURI="${ctx}/examManage/examPaperDetail.do"  size="${list.fullListSize}" pagesize="${list.objectsPerPage}"
				...
				<input type="radio" id="ids" name="ids"
                       value="${item.id}+${item.name}_${item.paper_mode}_${item.paper_score}_${item.question_num}_${fn:substring(item.create_date,0,10)}"/>
				</display:column>
				<display:column property="name" title="名称" style="width:40%;text-align:center"></display:column>
				...
				<display:column title="操作" style="width:20%;text-align:center;">
					<a href="${ctx}/paperManage/paperView.do?handle=viewPaper&paperId=${item.id}" class="">查看</a>
				</display:column>
			</display:table>
		</form>
...
<div class="toumingdu att_make01" style="display:none;z-index:10000;">
	<div class="tk_center09" style="margin:7% auto;">
		...
		<div class="xianshikuang">
			...
			<div class="xs_biaoti">
				<div class="fl xs_er">
					<p class="fl attr_xueke01">选择目录</p>
