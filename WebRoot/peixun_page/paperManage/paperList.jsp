	...
	<div class="center01">
		<div class="mt5 kit1_tiaojia">
			<div class="fr cas_anniu">
				<a href="${ctx}/peixun_page/paperManage/paperAdd.jsp" class="fl" style="width:80px;">新增</a>
			</div>
		</div>
		<div class="clear"></div>
		<form action="" id="formB"  method="post">
			<display:table id="item" list="${List}" requestURI="${ctx}/paperManage/paperList.do"  size="${List.fullListSize}" pagesize="${List.objectsPerPage}"
				...
				<display:column title="操作" style="width:20%;text-align:center;">
					<a href="javascript:deletePaper(${item.id});">删除</a>
					<a href="javascript:modifyPaper(${item.paper_mode},${item.id});">修改</a>
					<c:if test="${item.isnot_exp_paper eq 1}">
					<a href="${ctx}/paperManage/paperOut.do?paperId=${item.id}" class="">导出</a>
					<a href="${ctx}/paperManage/paperAnswer.do?paperId=${item.id}" class="">导出答案</a>
					</c:if>
					<a href="javascript:contral('${item.type_name}',${item.id}, ${item.type_id});" class="tiaozheng">调整</a>
		...
		<div style="padding-top:50px;">
			...
			<div class="mt30 lc_shengcheng">
				...
				<input type="hidden" id="curTypeId1" name ="typeId1" value="${typeId1}" />
				 <input type="hidden" id="paperId1" name ="paperId1" value="${typeId1}" />
			...
			<div class="cas_anniu" style="margin-top:50px;margin-left:170px;">
				...
				<a href="javascript:void(0);" class="fl queren_cancel" style="width:70px;margin-left:40px;">取消</a>
			</div>
		</div>
		<div class="clear"></div>
	</div>
...
//YHQ，2017-05-17
function modifyPaper(paperMode,selid){	
	var url = "${ctx}/paperManage/paperList.do?method=usingExam&paperId="  +selid;
	$.ajax({
	    url:url,
	    type: 'POST',
	    dataType: 'text',
	    async:false,
	    success: function(result){
		   if(result == "noUsingPaper"){
				document.location.href = "${ctx}/paperManage/paperUpdate.do?mode=" + paperMode + "&paperId=" + selid ;
		   }else if(result == "usingPaper"){
			   alert('考试已被使用，不能修改！');
		   }
	    }
	});			
