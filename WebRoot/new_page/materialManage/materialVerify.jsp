...
<div>
	...
	<div class="center">
		...
		<form name="sfrm" id="sfrm" action="${ctx}/material/MaterialManage.do?handle=main2&checked=${checked}" method="POST">
			<div class="tk_zuo">
...
<%-- 						<input type="text"  id="ICD" name="ICD" class="fl tik_select" readonly value="${ICD}"/>
 --%>					</div>
					...
					<div class="tik_xuanze">
					<div class="mt10 fl tk_tixing" style="margin-left: 28px;">
						<em class="fl">类型：</em>
						<div class="fl tik_select">
					...
					<!-- <div class="tik_xuanze"> -->
						<div class="mt10 fl tk_tixing" style="margin-left: 10px;">
							<em class="fl">入库时间：</em>
							<input type="text"  class="fl tik_select" style="width: 85px;" name="upload_date" value="${upload_date}" onClick="WdatePicker()" readonly="readonly"/>
							<em class="fl" style="padding:0px 3px 0px 3px;">至</em>
							<input type="text"  class="fl tik_select" style="width: 85px;" name="upload_date1" value="${upload_date1}" onClick="WdatePicker()" readonly="readonly"/>
						</div>
						<div class="mt10 fl tk_tixing" style="margin-left:-10px;">
							<span class="fl">审核时间：</span>
							<input type="text"  class="fl tik_select" style="width:86px;" name="deli_date" value="${deli_date}" onClick="WdatePicker()" readonly="readonly"/>
							<em class="fl" style="padding:0px 3px 0px 3px;">至</em>
							<input type="text"  class="fl tik_select" style="width:86px;" name="deli_date1" value="${deli_date1}" onClick="WdatePicker()" readonly="readonly"/>
						</div>
					<!-- </div> -->
					<div class="clear"></div>
				</div>
...
<%-- 				<em class="fl">排序规则：</em>
			...
			<div class="fr cas_anniu">
				<a href="javascript:void(0);" onclick="updateState(4);" class="fl" style="width:95px;margin-right:15px;">合格</a>
				<a href="javascript:void(0);" onclick="updateState(3);" class="fl" style="width:95px;margin-right:15px;">不合格</a>
				<a href="javascript:void(0);" onclick="doLockStates();" class="fl" style="width:95px;margin-right:15px;">禁用素材</a>
				<a href="javascript:void(0);" onclick="exportMaterial();" class="fl" style="width:95px;">导出素材</a>
			</div>
			<ul class="cas1_xuanxing">
					<li class="" onclick="javascript:location.href='${ctx}/material/MaterialVerify.do?handle=main2&checked=1'">已审核</li>
					<li class="action" onclick="javascript:location.href='${ctx}/material/MaterialVerify.do?handle=main2'">待审核</li>
			</ul>
		</div>
		<div class="clear"></div> 
			<display:table name="mtlList" requestURI="${ctx}/material/MaterialManage.do" id="p" class="mt10 cas_table" size="${totalCount}" pagesize="20"  sort="list" defaultsort="1">
				...
				<display:column title="学科" style="width:8%;" >
 				...
 				<display:column title="审核时间" style="width:8%;" sortProperty="deli_date" sortable="true">
 					<c:if test="${p.state != 5}">${p.deli_date}</c:if>
 				</display:column>
 				...
 				<display:column title="操作" style="width:13%;" >
					<a href="${ctx}/material/MaterialManage.do?mode=detail&spec=info&id=${p.id}" >查看</a>
					<a href="javascript:doLock(${p.id});" >
					<c:if test="${p.state==5}">启用</c:if>
					<c:if test="${p.state!=5}">禁用</c:if>
					</a>
					<a href="javascript:doDelMat(${p.id}, ${p.state});" >删除</a>
...
$(function(){
	...
	//select menu
		...
		if ('${checked}' == '1') {
			$('.cas1_xuanxing').find('li').eq(1).removeClass("action");
			$('.cas1_xuanxing').find('li').eq(0).addClass("action");
		}
...
function updateState(state) {
	if (!checkMaterialIds()) {
		 if(state == 4) 	  alert("请选择素材!");
		 else if(state == 3) alert("请选择素材!");
		 else if(state == 5) alert("请选择要禁用的素材!");
         return;
     }
	 ...
	 if(state != 3 || opinion != ""){
		...
		$.ajax({
			...
			success : function (b){
				...
				{
					if (eval(b)!=count)
						alert(count+'个素材中'+b+'个素材审核成功，其他'+eval(count-b)+'个素材因为状态素材不能审核！');
					else
						alert(count+'个素材审核成功');
			      	document.location.href = document.location.href.split("#")[0];
				}
