...
<script type="text/javascript" src="${ctx}/new_page/js/jquery.js"></script>
	...
	<body class = "bjs">
		<div class="center">
		<form action="${ctx}/caseManage/caseManage2.do?type=${type}" method="post" name="searchFrm" id="searchFrm">
	...
	<div class="tk_center01">
		<c:if test="${type != 1}">
			<ul class="cas1_xuanxing">
					...
					<li class="<c:if test="${type==2}">action</c:if>" onclick="javascript:location.href='${ctx}/caseManage/caseManage.do?type=2'">待审核</li>
			</ul>
		</c:if>
		<div class="clear"></div>
			...
			<display:table name="casePatientList" requestURI="${ctx}/caseManage/caseManage.do" id="p" class="mt10 cas_table" size="${totalCount}" pagesize="20"  sort="list" defaultsort="1">
				...
				<display:column title="性别" sortable="true" style="width:5%;" sortProperty="p.sex">
 				...
 				<display:column title="操作" style="width:12%;">
					...
					<a href="javascript:selectBL('${p.caseObject.id}','${p.patientObject.PName} ${p.propNames}')" >选择</a>
 ...
 * 说      明：选择病例
 */
 function selectBL(blId,titleVal){
		if(blId!='' && blId!=null){
			var html = "<img src=\"http://101.200.85.213:8090/peixun/images/bingli.jpg\" width=\"100\" height=\"100\" _url='"+blId+"' alt=\"bingli\" title=\"" + titleVal + "\" onclick=\"preview(" + blId + ",'bingli');\">"; //是后台的地址而非资源管理的后台
	     window.parent.ue.execCommand( 'inserthtml', html );
	     var dialog = window.parent.ue.getDialog("map");
	     //判断该单元任务点是否选中
	     if(window.parent.isRWD=='true'){
			//调用父页面的视频完成指标函数
			window.parent.comBingli();
		 }
	     dialog.close();
	}
}
