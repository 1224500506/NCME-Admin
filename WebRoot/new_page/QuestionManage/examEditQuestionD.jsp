...
<%--					<input type="hidden" name="cognize" id="cognize">	
			...
			<li>
				<div class="shitin_xinzeng01">
					...
					<textarea placeholder="最大500字" maxlength="500" onpropertychange="if(value.length>500) value=value.substr(0,500)" onpropertychange="if(value.length>500) value=value.substr(0,500)" onKeyup="javascript:view_count(this);" name="content" cols="" rows="" class="fl tki_bianji01"  style="width:580px;" id="content">${quest.content}</textarea>
				...
				<div class="mt10 shitin_xinzeng01">
					...
					<div class="fl semiOr" id="key_a">
						<div class="tik_danxuan">
							<p class="fl">
								<input type="radio" value="1" name="answer_key" <c:if test = "${answer_key == 1}"> checked</c:if> id="answer_key" class="fl"/>正确
								
							</p>
							<p class="fl">
								<input type="radio" value="2" name="answer_key" <c:if test = "${answer_key == 0}"> checked</c:if> id="answer_key" class="fl"/>错误
