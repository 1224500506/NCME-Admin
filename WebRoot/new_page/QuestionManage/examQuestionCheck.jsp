...
<div>
	...
	<div class="tk_center01" style="min-height:40px;">
		<div class="tk_zuo">
			<form action="${ctx}/questionManage/questionManage.do?handle=main2&viewCheck=${viewCheck}" method="post" name="fmx" id="fmx">			
					...
					<div class="mt10 mll8 fl tk_tixing">
						...
						<div class="fl tik_select">
							...
							<ul class="fl tk_list_1 tk_list" style="display:none">
								<c:if test="${viewCheck eq 'ok'}"><li>请选择</li><li>合格</li><li>不合格</li><li>禁用</li></c:if>
								<c:if test="${viewCheck eq 'no'}"><li>请选择</li><li>未审核</li></c:if>								
							</ul>							
						</div>
					</div>
					<div class="mt10 mll8 fl tk_tixing">
...
<%-- 				<em class="fl">排序规则：</em>
			...
			<div class="fr cas_anniu">
                <c:if test="${viewCheck=='no'}"><a href="javascript:void(0);" onclick="updateState(2);" class="fl" style="width:95px;margin-right:15px;">合格</a>
                    <a href="javascript:void(0);" onclick="updateState(3);" class="fl" style="width:95px;margin-right:15px;">不合格</a></c:if>
				<a href="javascript:void(0);" onclick="updateState(4);" class="fl" style="width:95px;margin-right:15px;">禁用</a>
				<a href="javascript:void(0);" onclick="exportQuestions();" class="fl" style="width:95px;">导出试题</a>
			</div>
			<ul class="cas1_xuanxing">
					<li class="action" onclick="javascript:qualify_OK();">已审核</li>
					<li class="" onclick = "javascript:qualify_No();">待审核</li>
			</ul>
		</div>
		<div class="clear"></div>
		...
		<table class="mt10 cas_table">
			...
			<display:table name="questionList" id="qt" style="width:100%;" class="mt10 cas_table" requestURI="${ctx}/questionManage/questionManage.do" decorator="com.hys.exam.displaytag.OverOutWrapper" >
				...
				<display:column sortable="true" title="题型" style="width:8%;text-align:center">
				<c:choose>
				...
				<display:column title="审核意见" style="width:15%;">
					<c:if test="${qt.state==3}">${qt.opinion}</c:if>
				</display:column>
				...
				<display:column title="操作" style="width:120;text-align:center">
					...
					<c:if test="${qt.state!=4}">
						<a href="javascript:updateStateOne(${qt.id},4);">禁用</a>
					</c:if>
					<c:if test="${qt.state==4}">
						<a href="javascript:updateStateOne(${qt.id},0);">启用</a>
					</c:if>
...
$(function(){
	//select menu
		...
		if ('${viewCheck}' == 'ok') {
			...
			$('.cas1_xuanxing').find('li').eq(0).removeClass("action");
			$('.cas1_xuanxing').find('li').eq(1).addClass("action");
		}
...
function updateStateOne(id, state){
...
function updateState(state) {
	if (!checkItemidArr()) {
		 if(state == 2) 	  alert("请选择试题!");
		 else if(state == 3) alert("请选择试题!");
		 else if(state == 4) alert("请选择要禁用的试题!");
         return false;
     }
	...
	if(state != 3 || opinion != ""){
		...
		$.ajax({
			...
			success : function (count){
				if (eval(count)!=allcount)
					alert(allcount+'个试题中'+count+'个试题审核成功，其他'+eval(allcount-count)+'个试题因为状态问题不能审核！');
				else
					alert(allcount+'个试题审核成功');
			    questionSearch();
			},
			error: function(){
			   	  alert('失败.请检查属性的关联!');
...
function onChange(orderBy) {
	window.location.href = "${ctx}/questionManage/questionManage.do?handle=main2&orderBy=" + orderBy;
}
...
function qualify_OK(){
	/* $('#define').show();
	$('#indefine').hide(); */
	window.location.href = '${ctx}/questionManage/questionManage.do?handle=main2&viewCheck=ok';
}
function qualify_No(){
	/* $('#indefine').show();
	$('#define').hide(); */
	window.location.href = '${ctx}/questionManage/questionManage.do?handle=main2&viewCheck=no';
}
