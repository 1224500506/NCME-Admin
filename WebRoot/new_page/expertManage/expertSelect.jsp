...
// 按数组接收  
String temp[] = request.getParameterValues("selected") ; 
...
%>
<input type="hidden" id="selected" value="<%=sb.toString()%>"/>
...
<div>
	...
	<div class="center">
		...
		<form id="sfrm" action="${ctx}/expert/ExpertManage.do?mode=select&gid=${gid}" method="POST">
			...
			<div class="tk_zuo">
				...
				<div class="mt10 mll8 fl tk_tixing">
					...
					<input type="hidden" id="pNames"  name="propNames" value="${info.propNames}" />
					<div class="fl tik_select01 takuang_xk" id="groupNames">${info.propNames}</div>
				</div>
			</div>
			<div class="clear"></div>
		...
		<div class="clear"></div> 
			<display:table name="extList" requestURI="${ctx}/expert/ExpertManage.do" id="p" class="mt10 cas_table" size="${totalCount}" pagesize="20">
				<display:column title="<input type='checkbox' class='chkall' id='chkall' value=''>" style="width:5%;text-align:center">
					<input type="checkbox" class='chk' name="eid" value="${p.id}" id="ecid"/>
					<input type="hidden" value="${p.groupIds}">
				</display:column>
				<display:column title="专家姓名" style="width:8%;" property="name"></display:column>
				<display:column title="职务" style="width:8%;" property="officeNames" ></display:column>
				<%--  
				...
				<display:column title="任职状态" style="width:5%;">
 				...
 				<display:column title="固定电话" style="width:10%;" property="phone1"></display:column>
 				<display:column title="手机号码" style="width:10%;" property="phone2"></display:column>
...
/* 		$(".fenye a").click(function(){	
			$('.chk:checked').each(function(){
             ...
             $(this).attr("href",val+'&selected='+selids);
		}) */
	//add expert
		$('.tianjia').click(function(){
			...
			var pageSelectedID=$("#selected").val();
			var url = '${ctx}/expert/ExpertSubGroup.do?mode=addMember';
			var params = 'gid=' + ${gid};
			params+= '&expertIds=' +selids+","+pageSelectedID;
